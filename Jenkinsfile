pipeline {
  ...
  environment {
        GIT_NAME = "eea.fise.policy"
        SONARQUBE_TAGS = "www.eea.europa.eu,forest.eea.europa.eu"
    }
  ...
  stages {
          ...
          "Ruff": {
            node(label: 'docker') {
              script {
                ...
                sh '''cp -rf eea.fise.policy/* .'''
                sh '''rm -rf eea.fise.policy'''
    ...
    //       //     script {
    //       //       checkout scm
    //       //       sh '''docker run -i --rm --name="$BUILD_TAG-plone6" -v $(pwd):/app/src/$GIT_NAME -e ADDONS="/app/src/$GIT_NAME[test]" -e DEVELOP="/app/src/$GIT_NAME" eeacms/forest-backend bin/zope-testrunner --auto-color --auto-progress --test-path /app/src/$GIT_NAME'''
    //       //     }
    //       //   }
    //       // }
    //     )
